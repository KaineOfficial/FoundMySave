...
powershell -File tools/smoke-test.ps1              # checks a real window actually appears
```
...
`tools/smoke-test.ps1` runs the executable and checks that a real window appears. A living process proves nothing: if a XAML resource is missing, WPF fails while loading and only an error box is left.
...
tools/smoke-test.ps1      checks the window actually opens
```
